...
final class SwipeProgressBar {
    ...
    void draw(Canvas canvas) {
        ...
        if (this.mRunning || this.mFinishTime > 0) {
            ...
            float rawProgress = ((float) ((now - this.mStartTime) % 2000)) / TitleBar.BACKBTN_LEFT_MARGIN;
            if (!this.mRunning) {
            ...
            ViewCompat.postInvalidateOnAnimation(this.mParent, this.mBounds.left, this.mBounds.top, this.mBounds.right, this.mBounds.bottom);
        } else if (this.mTriggerPercentage > 0.0f && ((double) this.mTriggerPercentage) <= WeightedLatLng.DEFAULT_INTENSITY) {
            drawTrigger(canvas, cx, cy);
        }
        canvas.restoreToCount(restoreCount);
    }
