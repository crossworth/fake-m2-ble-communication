...
public class CoordinateConvert {
    ...
    public static double[] fromSeveralGpsToAMap(String str) {
        try {
            ...
            for (int i = 0; i < length / 2; i++) {
                double[] a = C0471u.m2019a(Double.parseDouble(split[i * 2]), Double.parseDouble(split[(i * 2) + 1]));
                dArr[i * 2] = a[0];
                dArr[(i * 2) + 1] = a[1];
            }
            return dArr;
    ...
    public static double[] fromSeveralGpsToAMap(double[] dArr) {
        try {
            ...
            for (int i = 0; i < length / 2; i++) {
                double[] a = C0471u.m2019a(dArr[i * 2], dArr[(i * 2) + 1]);
                dArr2[i * 2] = a[0];
                dArr2[(i * 2) + 1] = a[1];
            }
            return dArr2;
