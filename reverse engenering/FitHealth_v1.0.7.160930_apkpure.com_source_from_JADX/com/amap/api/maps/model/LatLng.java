...
public final class LatLng implements Parcelable, Cloneable {
    ...
    private static DecimalFormat f869a = new DecimalFormat("0.000000", new DecimalFormatSymbols(Locale.US));
    ...
    public LatLng(double d, double d2, boolean z) {
        if (z) {
            if (-180.0d > d2 || d2 >= 180.0d) {
                this.longitude = m1089a(((((d2 - 180.0d) % 360.0d) + 360.0d) % 360.0d) - 180.0d);
            } else {
                this.longitude = m1089a(d2);
            }
            if (d < -90.0d || d > 90.0d) {
                try {
            ...
            this.latitude = m1089a(Math.max(-90.0d, Math.min(90.0d, d)));
            return;
        }
        this.latitude = d;
        this.longitude = d2;
    ...
    private static double m1089a(double d) {
        return Double.parseDouble(f869a.format(d));
    }
    ...
    public String toString() {
        return "lat/lng: (" + this.latitude + SeparatorConstants.SEPARATOR_ADS_ID + this.longitude + SocializeConstants.OP_CLOSE_PAREN;
    }
