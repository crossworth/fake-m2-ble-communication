...
public class CameraAnimator {
    ...
    private int getSplineFlingDuration(float f) {
        return (int) (Math.exp(getSplineDeceleration(f) / (((double) DECELERATION_RATE) - WeightedLatLng.DEFAULT_INTENSITY)) * 1000.0d);
    }

    private double getSplineFlingDistance(float f) {
        return Math.exp(getSplineDeceleration(f) * (((double) DECELERATION_RATE) / (((double) DECELERATION_RATE) - WeightedLatLng.DEFAULT_INTENSITY))) * ((double) (this.mFlingFriction * this.mPhysicalCoeff));
    }
