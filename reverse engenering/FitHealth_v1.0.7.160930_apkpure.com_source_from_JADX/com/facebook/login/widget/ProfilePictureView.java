...
public class ProfilePictureView extends FrameLayout {
    ...
    class C16641 implements Callback {
        C16641() {
        }
    ...
    private void initialize(Context context) {
        this.f2304R = ResContainer.get(context);
        removeAllViews();
    ...
    private void parseAttributes(AttributeSet attrs) {
        TypedArray a = getContext().obtainStyledAttributes(attrs, ResContainer.getStyleableArrts(getContext(), "com_facebook_profile_picture_view"));
        setPresetSize(a.getInt(this.f2304R.styleable("com_facebook_profile_picture_view_com_facebook_preset_size"), -1));
        this.isCropped = a.getBoolean(this.f2304R.styleable("com_facebook_profile_picture_view_com_facebook_is_cropped"), true);
        a.recycle();
    }
    ...
    private void setBlankProfilePicture() {
        ...
        if (this.customizedDefaultProfilePicture == null) {
            ...
            if (isCropped()) {
                blankImageResource = this.f2304R.drawable("com_facebook_profile_picture_blank_square");
            } else {
                blankImageResource = this.f2304R.drawable("com_facebook_profile_picture_blank_portrait");
            }
    ...
    private void sendImageRequest(boolean allowCachedResponse) {
        ImageRequest request = new Builder(getContext(), ImageRequest.getProfilePictureUri(this.profileId, this.queryWidth, this.queryHeight)).setAllowCachedRedirects(allowCachedResponse).setCallerTag(this).setCallback(new C16641()).build();
        if (this.lastRequest != null) {
            ImageDownloader.cancelRequest(this.lastRequest);
        }
    ...
    private int getPresetSizeInPixels(boolean forcePreset) {
        ...
        switch (this.presetSizeType) {
            case -4:
                dimensionId = this.f2304R.dimen("com_facebook_profilepictureview_preset_size_large");
                break;
            case -3:
                dimensionId = this.f2304R.dimen("com_facebook_profilepictureview_preset_size_normal");
                break;
            case -2:
                dimensionId = this.f2304R.dimen("com_facebook_profilepictureview_preset_size_small");
                break;
            case -1:
                if (forcePreset) {
                    dimensionId = this.f2304R.dimen("com_facebook_profilepictureview_preset_size_normal");
