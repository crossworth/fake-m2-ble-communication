...
public class WXImageObject implements IMediaObject {
    ...
    public boolean checkArgs() {
        if ((this.imageData == null || this.imageData.length == 0) && ((this.imagePath == null || this.imagePath.length() == 0) && (this.imageUrl == null || this.imageUrl.length() == 0))) {
            C0765a.m2514a(TAG, "checkArgs fail, all arguments are null");
            return false;
        } else if (this.imageData != null && this.imageData.length > CONTENT_LENGTH_LIMIT) {
            C0765a.m2514a(TAG, "checkArgs fail, content is too large");
            return false;
        } else if (this.imagePath != null && this.imagePath.length() > NetworkConstants.DOWNLOAD_BUFFER_SIZE) {
            C0765a.m2514a(TAG, "checkArgs fail, path is invalid");
            return false;
        } else if (this.imagePath != null && getFileSize(this.imagePath) > CONTENT_LENGTH_LIMIT) {
            C0765a.m2514a(TAG, "checkArgs fail, image content is too large");
            return false;
        } else if (this.imageUrl == null || this.imageUrl.length() <= NetworkConstants.DOWNLOAD_BUFFER_SIZE) {
            return true;
        } else {
            C0765a.m2514a(TAG, "checkArgs fail, url is invalid");
            return false;
        }
    }
