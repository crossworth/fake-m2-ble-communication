...
public abstract class BaseNCodec {
    ...
    private void resizeBuffer() {
        ...
        Object obj = new byte[(this.mBuffer.length * 2)];
    ...
    public boolean isInAlphabet(byte[] bArr, boolean z) {
        ...
        while (i < bArr.length) {
            if (!isInAlphabet(bArr[i])) {
                ...
                if (!(bArr[i] == (byte) 61 || isWhiteSpace(bArr[i]))) {
                    return false;
                }
            }
            i++;
    ...
    protected boolean containsAlphabetOrPad(byte[] bArr) {
        ...
        int i = 0;
        while (i < bArr.length) {
            if ((byte) 61 == bArr[i] || isInAlphabet(bArr[i])) {
                return true;
            }
            i++;
        }
    ...
    public long getEncodedLength(byte[] bArr) {
        ...
        if (this.mLineLength > 0) {
            return length + ((((((long) this.mLineLength) + length) - 1) / ((long) this.mLineLength)) * ((long) this.mChunkSeparatorLength));
        }
        return length;
    }
}
