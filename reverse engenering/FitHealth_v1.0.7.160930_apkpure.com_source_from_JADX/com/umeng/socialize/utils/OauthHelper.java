...
public class OauthHelper {
    ...
    public static boolean isAuthenticated(Context context, SHARE_MEDIA share_media) {
        if (context == null) {
            Log.m3260w(TAG, "context对象为空，请传递一个非空Context对象");
            return false;
        } else if (share_media == null || share_media == SHARE_MEDIA.GENERIC) {
            Log.m3260w(TAG, "传递的检测授权平台无效");
            return false;
        ...
        try {
            for (SHARE_MEDIA share_media : SHARE_MEDIA.getDefaultPlatform()) {
                if (isAuthenticated(context, share_media)) {
                    ...
                    Log.m3254i(TAG, "found platform " + share_media.toString() + " usid=" + usid);
                } else {
                    Log.m3254i(TAG, "No found oauthed platform " + share_media.toString());
                }
            }
            Log.m3254i(TAG, "found platform count " + hashMap.size());
            return hashMap;
        } catch (NullPointerException e) {
            Log.m3254i(TAG, "no authenticated platform.......");
            return null;
        }
    }
    ...
    public static void saveTokenExpiresIn(Context context, SHARE_MEDIA share_media, String str) {
        if (context == null) {
            Log.m3260w(TAG, "context is null when save expires in");
            return;
        }
        String share_media2 = share_media.toString();
        if (TextUtils.isEmpty(share_media2)) {
            Log.m3260w(TAG, "platform is null when save expires in ");
            ...
            if (longValue <= 0) {
                Log.m3260w(TAG, share_media2 + " expires in is " + longValue);
                return;
            }
            longValue += System.currentTimeMillis() / 1000;
            Editor edit = context.getSharedPreferences(SocializeConstants.SOCIAL_PREFERENCE_EXPIRE_IN, 0).edit();
            edit.putLong(share_media2, longValue);
            edit.commit();
        }
    }
    ...
    public static long getTokenExpiresIn(Context context, SHARE_MEDIA share_media) {
        if (context == null) {
            Log.m3260w(TAG, "context is null when obtain expires in");
            return 0;
        }
        Object share_media2 = share_media.toString();
        if (!TextUtils.isEmpty(share_media2)) {
            return context.getSharedPreferences(SocializeConstants.SOCIAL_PREFERENCE_EXPIRE_IN, 0).getLong(share_media2, 0);
        }
        Log.m3260w(TAG, "platform is null when save expires in ");
        return 0;
    }
    ...
    public static void removeTokenExpiresIn(Context context, SHARE_MEDIA share_media) {
        if (context == null) {
            Log.m3260w(TAG, "context is null when removing expires in");
            return;
        }
        Object share_media2 = share_media.toString();
        if (TextUtils.isEmpty(share_media2)) {
    ...
    public static String getUsid(Context context, SHARE_MEDIA share_media) {
        ...
        return sharedPreferences.getString(share_media2, "");
    }
    ...
    public static void setUsid(Context context, SHARE_MEDIA share_media, String str) {
        ...
        synchronized (sharedPreferences) {
            ...
            if (!TextUtils.isEmpty(share_media2)) {
                ...
                edit.commit();
                Log.m3248d(TAG, "Save platform " + share_media2 + "   " + str);
            }
        }
    }
    ...
    public static String[] getAccessToken(Context context, SHARE_MEDIA share_media) {
        ...
        share_media2 = share_media2 + "_as";
        if (!sharedPreferences.contains(str) || !sharedPreferences.contains(share_media2)) {
            return new String[0];
        }
        str = sharedPreferences.getString(str, "");
        share_media2 = sharedPreferences.getString(share_media2, "");
        return new String[]{str, share_media2};
    }

    public static void saveQQAccessToken(Context context, JSONObject jSONObject) {
        saveQQAccessToken(context, jSONObject.optString("access_token", ""), jSONObject.optString("openid", ""), jSONObject.optString("expires_in", ""));
    }

    public static void saveQQAccessToken(Context context, Bundle bundle) {
        saveQQAccessToken(context, bundle.getString("access_token"), bundle.getString("openid"), bundle.getString("expires_in"));
    }
    ...
    public static String[] getAccessTokenForQQ(Context context) {
        ...
        Log.m3248d(TAG, "get QQ Token." + string);
    ...
    public static void remove(Context context, SHARE_MEDIA share_media) {
        ...
        synchronized (sharedPreferences) {
            ...
            edit.commit();
            Log.m3248d(TAG, "Remove platform " + share_media.toString());
        }
    ...
    public static void saveRefreshToken(Context context, SHARE_MEDIA share_media, String str) {
        Editor edit = context.getSharedPreferences(SocializeConstants.SOCIAL_PREFERENCE_REFRESH_TOKEN, 0).edit();
    ...
    public static void deleteRefreshToken(Context context, SHARE_MEDIA share_media) {
        ...
        edit.remove(share_media.toString());
        edit.commit();
        edit = context.getSharedPreferences(SocializeConstants.SOCIAL_PREFERENCE_REFRESH_TOKEN_EXPIRES, 0).edit();
        edit.remove(share_media.toString());
        edit.commit();
    }
    ...
    public static void saveRefreshTokenExpires(Context context, SHARE_MEDIA share_media, long j) {
        ...
        Editor edit = context.getSharedPreferences(SocializeConstants.SOCIAL_PREFERENCE_REFRESH_TOKEN_EXPIRES, 0).edit();
    ...
    public static boolean isRefreshTokenNotExpired(Context context, SHARE_MEDIA share_media) {
        if (context.getSharedPreferences(SocializeConstants.SOCIAL_PREFERENCE_REFRESH_TOKEN_EXPIRES, 0).getLong(share_media.toString(), 0) > System.currentTimeMillis() / 1000) {
            return true;
        }
        return false;
    }
