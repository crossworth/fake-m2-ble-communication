...
public class ResUtil {
    ...
    final class C09831 implements Runnable {
        ...
        protected Drawable doInBackground(Object... objArr) {
            ...
            if (this.mCacheFile == null || !this.mCacheFile.exists()) {
                try {
                    ...
                    Log.m3248d(ResUtil.TAG, "get drawable from net else file.");
                    return drawable;
                } catch (Exception e2) {
                    Log.m3261w(ResUtil.TAG, e2.toString(), e2);
                    return null;
                }
            }
            ...
            Log.m3248d(ResUtil.TAG, "get drawable from cacheFile.");
            return drawable;
        }
    }
        ...
        try {
            ...
            if (file2.exists()) {
                if (dirSize(file2.getCanonicalFile()) > j) {
                    new Thread(new C09831(file2)).start();
                }
            } else if (!file2.mkdirs()) {
                Log.m3251e(TAG, "Failed to create directory" + file2.getAbsolutePath() + ". Check permission. Make sure WRITE_EXTERNAL_STORAGE is added in your Manifest.xml");
            }
            ...
            try {
                ...
                while (true) {
                    ...
                    if (read != -1) {
                        ...
                        file.renameTo(file3);
                        Log.m3254i(TAG, "download img[" + str + "]  to " + file3.getCanonicalPath());
                        return file3.getCanonicalPath();
                    }
                }
            } catch (Exception e2) {
                e = e2;
                Log.m3254i(TAG, e.getStackTrace().toString() + "\t url:\t" + str);
                file.deleteOnExit();
                return null;
            }
        } catch (Exception e3) {
    ...
    private static synchronized void doBind(Context context, ImageView imageView, Drawable drawable, boolean z, BindDrawableListener bindDrawableListener, Animation animation, String str) {
        synchronized (ResUtil.class) {
            if (drawable != null) {
                try {
                    drawable = new BitmapDrawable(getRoundedCornerBitmap(((BitmapDrawable) drawable).getBitmap()));
                } catch (Exception e) {
                    Log.m3252e(TAG, "bind failed", e);
                    if (bindDrawableListener != null) {
                        bindDrawableListener.onEnd(STATUS.FAIL);
                    }
                }
            ...
            if (drawable == null || imageView == null) {
                ...
                Log.m3260w(TAG, "bind drawable failed. drawable [" + drawable + "]  imageView[+" + imageView + "+]");
            } else {
    ...
    private static Drawable createFromPathBuffer(String str) {
        ...
        try {
            ...
            Log.m3260w(TAG, "Resutil fetchImage OutOfMemoryError:" + e.toString());
        }
        return drawable;
    }
    ...
    private static Bitmap getRoundedCornerBitmap(Bitmap bitmap) {
        try {
            ...
            Log.m3260w(TAG, "Cant`t create round corner bitmap. [OutOfMemoryError] ");
            return null;
        }
    }
}
