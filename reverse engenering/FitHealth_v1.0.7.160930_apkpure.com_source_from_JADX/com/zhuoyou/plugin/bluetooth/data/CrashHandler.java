...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    public void collectDeviceInfo(Context ctx) {
        try {
            ...
            if (pi != null) {
                ...
                this.infos.put(PromConstants.PROM_HTML5_INFO_VERSION_CODE, versionCode);
            }
    ...
    private String saveCrashInfo2File(Throwable ex, String logPath) {
        ...
        try {
            String fileName = this.mContext.getPackageName() + "_crash-" + this.formatter.format(new Date()) + SocializeConstants.OP_DIVIDER_MINUS + System.currentTimeMillis() + SocializeConstants.OP_DIVIDER_MINUS + ((String) this.infos.get(PromConstants.PROM_HTML5_INFO_VERSION_CODE)) + ".log";
    ...
    public void deleteFiles(File[] files, String fileIndexs) {
        if (!TextUtils.isEmpty(fileIndexs)) {
            try {
                for (String str : fileIndexs.split(SeparatorConstants.SEPARATOR_ADS_ID)) {
    ...
    private boolean deleteFileByVersionCode(File file, int code) {
        if (file == null || file.getName().endsWith(SocializeConstants.OP_DIVIDER_MINUS + code + ".log")) {
            return false;
        }
        file.delete();
        return true;
