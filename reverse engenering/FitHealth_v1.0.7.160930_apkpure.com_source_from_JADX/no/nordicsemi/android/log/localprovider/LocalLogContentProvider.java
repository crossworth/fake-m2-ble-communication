...
public abstract class LocalLogContentProvider extends ContentProvider {
    ...
    private static final ProjectionMap sLogColumns = ProjectionMap.builder().add("_id").add(LogColumns.SESSION_ID).add(LogColumns.LEVEL).add(LogColumns.TIME).add("data").build();
    private static final ProjectionMap sSessionColumns = ProjectionMap.builder().add("_id").add("key").add("name").add(SessionColumns.CREATED_AT).build();
        ...
        switch (match) {
            ...
            case SESSION_ID_LOG_CONTENT /*1023*/:
                ...
                if (match == SESSION_ID_LOG_CONTENT) {
                    ...
                    sessionSelArgs[0] = id;
                    Cursor c = db.query("sessions", sessionProjection, "_id=?", sessionSelArgs, null, null, null);
                    try {
                        ...
                        c = query(uri2, sQLiteDatabase, qb, new String[]{LogColumns.TIME, LogColumns.LEVEL, "data"}, selection, selectionArgs, "time ASC");
                        try {
    ...
    private long insertSession(Uri uri, ContentValues values) {
        ...
        return ((LocalLogDatabaseHelper) this.mLocalDatabaseHelper.get()).getWritableDatabase().insert("sessions", null, this.mValues);
    }
