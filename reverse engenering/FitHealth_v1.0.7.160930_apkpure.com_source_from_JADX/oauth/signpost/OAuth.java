...
public class OAuth {
    ...
    public static String prepareOAuthHeader(String... kvPairs) {
        ...
        for (int i = 0; i < kvPairs.length; i += 2) {
            ...
            sb.append(percentEncode(kvPairs[i]) + "=\"" + (kvPairs[i].startsWith("oauth_") ? percentEncode(kvPairs[i + 1]) : kvPairs[i + 1]) + "\"");
        }
        return sb.toString();
    }
    ...
    public static HttpParameters oauthHeaderToParamsMap(String oauthHeader) {
        ...
        if (oauthHeader != null && oauthHeader.startsWith("OAuth ")) {
            for (String keyValuePair : oauthHeader.substring("OAuth ".length()).split(SeparatorConstants.SEPARATOR_ADS_ID)) {
