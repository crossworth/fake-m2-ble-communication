...
public class OAuthAuthorization implements Authorization, Serializable, OAuthSupport {
    ...
    private static final HttpParameter OAUTH_SIGNATURE_METHOD = new HttpParameter(OAuth.OAUTH_SIGNATURE_METHOD, HttpHeaderFactory.CONST_SIGNATURE_METHOD);
    ...
    public RequestToken getOAuthRequestToken(String callbackURL, String xAuthAccessType, String xAuthMode) throws TwitterException {
        ...
        if (callbackURL != null) {
            params.add(new HttpParameter(OAuth.OAUTH_CALLBACK, callbackURL));
        }
    ...
    public AccessToken getOAuthAccessToken(String oauthVerifier) throws TwitterException {
        ensureTokenIsAvailable();
        this.oauthToken = new AccessToken(http.post(this.conf.getOAuthAccessTokenURL(), new HttpParameter[]{new HttpParameter(OAuth.OAUTH_VERIFIER, oauthVerifier)}, this, null));
        return (AccessToken) this.oauthToken;
    }
    ...
    String generateAuthorizationHeader(String method, String url, HttpParameter[] params, String nonce, String timestamp, OAuthToken otoken) {
        ...
        oauthHeaderParams.add(new HttpParameter(OAuth.OAUTH_VERSION, "1.0"));
        if (otoken != null) {
        ...
        return "OAuth " + encodeParameters(oauthHeaderParams, SeparatorConstants.SEPARATOR_ADS_ID, true);
    }
    ...
    String generateSignature(String data, OAuthToken token) {
        try {
            ...
            if (token == null) {
                spec = new SecretKeySpec((HttpParameter.encode(this.consumerSecret) + "&").getBytes(), "HmacSHA1");
            } else {
                spec = token.getSecretKeySpec();
                if (spec == null) {
                    spec = new SecretKeySpec((HttpParameter.encode(this.consumerSecret) + "&" + HttpParameter.encode(token.getTokenSecret())).getBytes(), "HmacSHA1");
