...
class MaterialProgressDrawable extends Drawable implements Animatable {
    ...
    private final Callback mCallback = new C01443();
    boolean mFinishing;
    ...
    class C01443 implements Callback {
        C01443() {
        }
    ...
    public void updateSizes(@ProgressDrawableSize int size) {
        if (size == 0) {
            ...
            setSizeParameters(40.0d, 40.0d, 8.75d, 2.5d, TitleBar.SHAREBTN_RIGHT_MARGIN, 5.0f);
        }
    }
    ...
    private void updateRingColor(float interpolatedTime, Ring ring) {
        if (interpolatedTime > 0.75f) {
            ring.setColor(evaluateColorChange((interpolatedTime - 0.75f) / ChartZoomer.ZOOM_AMOUNT, ring.getStartingColor(), ring.getNextColor()));
        }
    }

    private void applyFinishTranslation(float interpolatedTime, Ring ring) {
        updateRingColor(interpolatedTime, ring);
        float targetRotation = (float) (Math.floor((double) (ring.getStartingRotation() / MAX_PROGRESS_ARC)) + WeightedLatLng.DEFAULT_INTENSITY);
    ...
    private void setupAnimators() {
        ...
        Animation animation = new Animation() {
            public void applyTransformation(float interpolatedTime, Transformation t) {
                ...
                ring.setRotation(startingRotation + (ChartZoomer.ZOOM_AMOUNT * interpolatedTime));
