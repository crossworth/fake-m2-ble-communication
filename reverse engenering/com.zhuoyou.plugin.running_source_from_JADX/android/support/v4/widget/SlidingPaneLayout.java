        ...
        for (i = 0; i < childCount; i++) {
            ...
            if (child.getVisibility() == 8) {
                ...
                if (lp.width == -2) {
                    ...
                    childWidthSpec = MeasureSpec.makeMeasureSpec(widthAvailable - horizontalMargin, DataConstants.BYTES_PER_GIGABYTE);
                } else {
                    childWidthSpec = MeasureSpec.makeMeasureSpec(lp.width, DataConstants.BYTES_PER_GIGABYTE);
                }
                if (lp.height == -2) {
                    childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, Integer.MIN_VALUE);
                } else if (lp.height == -1) {
                    childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, DataConstants.BYTES_PER_GIGABYTE);
                } else {
                    childHeightSpec = MeasureSpec.makeMeasureSpec(lp.height, DataConstants.BYTES_PER_GIGABYTE);
                }
                ...
                if (child.getVisibility() != 8) {
                    ...
                    if (child.getVisibility() != 8) {
                        ...
                        if (!canSlide || child == this.mSlideableView) {
                            if (lp.weight > 0.0f) {
                                if (lp.width != 0) {
                                    childHeightSpec = MeasureSpec.makeMeasureSpec(child.getMeasuredHeight(), DataConstants.BYTES_PER_GIGABYTE);
                                } else if (lp.height == -2) {
                                    childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, Integer.MIN_VALUE);
                                } else if (lp.height == -1) {
                                    childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, DataConstants.BYTES_PER_GIGABYTE);
                                ...
                                if (canSlide) {
                                    ...
                                    childWidthSpec = MeasureSpec.makeMeasureSpec(newWidth, DataConstants.BYTES_PER_GIGABYTE);
                                    if (measuredWidth != newWidth) {
                                        child.measure(childWidthSpec, childHeightSpec);
                                    }
                                } else {
                                    child.measure(MeasureSpec.makeMeasureSpec(measuredWidth + ((int) ((lp.weight * ((float) Math.max(0, widthRemaining))) / weightSum)), DataConstants.BYTES_PER_GIGABYTE), childHeightSpec);
                                }
                            }
                            ...
                            child.measure(MeasureSpec.makeMeasureSpec(fixedPanelWidthLimit, DataConstants.BYTES_PER_GIGABYTE), childHeightSpec);
                        }
                    }
                }
            }
