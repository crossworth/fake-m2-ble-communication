...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> callback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new C01931();
            ...
            class C01931 implements Runnable {
                ...
                public void run() {
                    SyncQueueItem msg = C01941.this.mQueue.next();
                    while (msg != null) {
                        switch (msg.what) {
                            case 1:
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> callback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new C01951();
            ...
            class C01951 implements Runnable {
                ...
                public void run() {
                    while (true) {
                        SyncQueueItem msg = C01962.this.mQueue.next();
                        if (msg != null) {
                            switch (msg.what) {
                                case 1:
                                    C01962.this.mQueue.removeMessages(1);
                                    callback.refresh(msg.arg1);
                                    break;
                                case 2:
                                    C01962.this.mQueue.removeMessages(2);
                                    C01962.this.mQueue.removeMessages(3);
                        ...
                        C01962.this.mBackgroundRunning.set(false);
                        return;
                    }
                }
            }
