...
public class ActionBarPolicy {
    ...
    public boolean hasEmbeddedTabs() {
        if (this.mContext.getApplicationInfo().targetSdkVersion >= 16) {
            return this.mContext.getResources().getBoolean(C0189R.bool.abc_action_bar_embed_tabs);
        }
    ...
    public int getTabContainerHeight() {
        ...
        if (!hasEmbeddedTabs()) {
            height = Math.min(height, r.getDimensionPixelSize(C0189R.dimen.abc_action_bar_stacked_max_height));
        }
        a.recycle();
        return height;
    }
