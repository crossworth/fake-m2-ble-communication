...
class AppCompatTextHelper {
    private static final int[] TEXT_APPEARANCE_ATTRS = new int[]{C0189R.attr.textAllCaps};
    ...
    void loadFromAttributes(AttributeSet attrs, int defStyleAttr) {
        ...
        if (!(this.mView.getTransformationMethod() instanceof PasswordTransformationMethod)) {
            ...
            if (ap != -1) {
                TypedArray appearance = context.obtainStyledAttributes(ap, C0189R.styleable.TextAppearance);
                if (appearance.hasValue(C0189R.styleable.TextAppearance_textAllCaps)) {
                    allCapsSet = true;
                    allCaps = appearance.getBoolean(C0189R.styleable.TextAppearance_textAllCaps, false);
                }
                appearance.recycle();
            }
