    ...
    void layoutChunk(Recycler recycler, State state, LayoutState layoutState, LayoutChunkResult result) {
        ...
        if (flexibleInOtherDir) {
            ...
            for (i = 0; i < count; i++) {
                ...
                spec = LayoutManager.getChildMeasureSpec(this.mCachedBorders[lp.mSpanIndex + lp.mSpanSize] - this.mCachedBorders[lp.mSpanIndex], DataConstants.BYTES_PER_GIGABYTE, 0, this.mOrientation == 0 ? lp.height : lp.width, false);
        ...
        int maxMeasureSpec = MeasureSpec.makeMeasureSpec(maxSize, DataConstants.BYTES_PER_GIGABYTE);
        for (i = 0; i < count; i++) {
            view = this.mSet[i];
