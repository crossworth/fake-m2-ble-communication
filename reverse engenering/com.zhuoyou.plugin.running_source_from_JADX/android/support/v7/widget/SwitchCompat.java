...
public class SwitchCompat extends CompoundButton {
    ...
    public SwitchCompat(Context context, AttributeSet attrs) {
        this(context, attrs, C0189R.attr.switchStyle);
    }
    ...
    public SwitchCompat(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TintTypedArray a = TintTypedArray.obtainStyledAttributes(context, attrs, C0189R.styleable.SwitchCompat, defStyleAttr, 0);
        this.mThumbDrawable = a.getDrawable(C0189R.styleable.SwitchCompat_android_thumb);
        if (this.mThumbDrawable != null) {
            this.mThumbDrawable.setCallback(this);
        }
        this.mTrackDrawable = a.getDrawable(C0189R.styleable.SwitchCompat_track);
        if (this.mTrackDrawable != null) {
            this.mTrackDrawable.setCallback(this);
        }
        this.mTextOn = a.getText(C0189R.styleable.SwitchCompat_android_textOn);
        this.mTextOff = a.getText(C0189R.styleable.SwitchCompat_android_textOff);
        this.mShowText = a.getBoolean(C0189R.styleable.SwitchCompat_showText, true);
        this.mThumbTextPadding = a.getDimensionPixelSize(C0189R.styleable.SwitchCompat_thumbTextPadding, 0);
        this.mSwitchMinWidth = a.getDimensionPixelSize(C0189R.styleable.SwitchCompat_switchMinWidth, 0);
        this.mSwitchPadding = a.getDimensionPixelSize(C0189R.styleable.SwitchCompat_switchPadding, 0);
        this.mSplitTrack = a.getBoolean(C0189R.styleable.SwitchCompat_splitTrack, false);
        int appearance = a.getResourceId(C0189R.styleable.SwitchCompat_switchTextAppearance, 0);
        if (appearance != 0) {
    ...
    public void setSwitchTextAppearance(Context context, int resid) {
        TypedArray appearance = context.obtainStyledAttributes(resid, C0189R.styleable.TextAppearance);
        ColorStateList colors = appearance.getColorStateList(C0189R.styleable.TextAppearance_android_textColor);
        if (colors != null) {
            this.mTextColors = colors;
        } else {
            this.mTextColors = getTextColors();
        }
        int ts = appearance.getDimensionPixelSize(C0189R.styleable.TextAppearance_android_textSize, 0);
        if (!(ts == 0 || ((float) ts) == this.mTextPaint.getTextSize())) {
            this.mTextPaint.setTextSize((float) ts);
            requestLayout();
        }
        setSwitchTypefaceByIndex(appearance.getInt(C0189R.styleable.TextAppearance_android_typeface, -1), appearance.getInt(C0189R.styleable.TextAppearance_android_textStyle, -1));
        if (appearance.getBoolean(C0189R.styleable.TextAppearance_textAllCaps, false)) {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        switch (getGravity() & BtManagerService.CLASSIC_CMD_GET_STEP) {
            case 16:
