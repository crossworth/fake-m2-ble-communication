...
public class DiskBasedCache implements Cache {
    ...
    public DiskBasedCache(File rootDirectory, int maxCacheSizeInBytes) {
        this.mEntries = new LinkedHashMap(16, ColumnChartData.DEFAULT_FILL_RATIO, true);
        this.mTotalSize = 0;
    ...
    public synchronized void clear() {
        synchronized (this) {
            ...
            this.mEntries.clear();
            this.mTotalSize = 0;
            VolleyLog.m46d("Cache cleared.", new Object[0]);
        }
    }
            ...
            if (entry2 != null) {
                ...
                try {
                    ...
                    try {
                        ...
                        cis = cis2;
                        try {
                            VolleyLog.m46d("%s: %s", file.getAbsolutePath(), e.toString());
                            remove(key);
                            if (cis != null) {
                                try {
                                    cis.close();
                        ...
                        e2 = e7;
                        cis = cis2;
                        VolleyLog.m46d("%s: %s", file.getAbsolutePath(), e2.toString());
                        remove(key);
                        if (cis != null) {
                            try {
                                cis.close();
    ...
    public synchronized void initialize() {
        ...
        synchronized (this) {
            if (this.mRootDirectory.exists()) {
                ...
                VolleyLog.m47e("Unable to create cache dir %s", this.mRootDirectory.getAbsolutePath());
            }
        }
        return;
        if (fis != null) {
    ...
    public synchronized void put(String key, Entry entry) {
        ...
        try {
            ...
            if (e.writeHeader(fos)) {
                ...
                fos.close();
                VolleyLog.m46d("Failed to write header for %s", file.getAbsolutePath());
                throw new IOException();
            }
        } catch (IOException e2) {
            if (!file.delete()) {
                VolleyLog.m46d("Could not clean up file %s", file.getAbsolutePath());
            }
        }
    }
    ...
    public synchronized void remove(String key) {
        ...
        removeEntry(key);
        if (!deleted) {
            VolleyLog.m46d("Could not delete cache entry for key=%s, filename=%s", key, getFilenameForKey(key));
        }
    }
    ...
    private void pruneIfNeeded(int neededSpace) {
        if (this.mTotalSize + ((long) neededSpace) >= ((long) this.mMaxCacheSizeInBytes)) {
            if (VolleyLog.DEBUG) {
                VolleyLog.m49v("Pruning old cache entries.", new Object[0]);
            }
            ...
            while (iterator.hasNext()) {
                ...
                if (getFileForKey(e.key).delete()) {
                    ...
                    VolleyLog.m46d("Could not delete cache entry for key=%s, filename=%s", e.key, getFilenameForKey(e.key));
                }
                iterator.remove();
                prunedFiles++;
            ...
            if (VolleyLog.DEBUG) {
                VolleyLog.m49v("pruned %d files, %d bytes, %d ms", Integer.valueOf(prunedFiles), Long.valueOf(this.mTotalSize - before), Long.valueOf(SystemClock.elapsedRealtime() - startTime));
            }
        }
    }
