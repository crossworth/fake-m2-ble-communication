...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    private static final int MSG_REFRESH_COMPLETE = 4097;
    private static final int REFRESH_COMPLETE_DELAY = 700;
    ...
    private OnHeaderSizeListener onHeaderSizeListener;
    ...
    class C10561 implements Runnable {
        ...
        public void run() {
            ...
            PullToRefreshBase.this.getLoadingLayoutProxy().resetLabel();
        }
    }
    ...
    class C10572 implements OnSmoothScrollFinishedListener {
        C10572() {
        }
    ...
    class C10594 implements OnSmoothScrollFinishedListener {
        C10594() {
        }
    ...
    public interface OnHeaderSizeListener {
        void onHeaderSize(int i);
    }
    ...
    public void setOnHeaderSizeListener(OnHeaderSizeListener listener) {
        this.onHeaderSizeListener = listener;
    }
    ...
    protected void onRefreshing(boolean doScroll) {
        ...
        if (!doScroll) {
            ...
            OnSmoothScrollFinishedListener listener = new C10572();
            switch (this.mCurrentMode) {
                case PULL_FROM_END:
    ...
    protected final void setHeaderScroll(int value) {
        ...
        if (this.onHeaderSizeListener != null) {
            this.onHeaderSizeListener.onHeaderSize(value);
        }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, C1064R.styleable.PullToRefresh);
        if (a.hasValue(C1064R.styleable.PullToRefresh_ptrMode)) {
            this.mMode = Mode.mapIntToValue(a.getInteger(C1064R.styleable.PullToRefresh_ptrMode, 0));
        }
        if (a.hasValue(C1064R.styleable.PullToRefresh_ptrAnimationStyle)) {
            this.mLoadingAnimationStyle = AnimationStyle.mapIntToValue(a.getInteger(C1064R.styleable.PullToRefresh_ptrAnimationStyle, 0));
        }
        ...
        Drawable background;
        if (a.hasValue(C1064R.styleable.PullToRefresh_ptrRefreshableViewBackground)) {
            background = a.getDrawable(C1064R.styleable.PullToRefresh_ptrRefreshableViewBackground);
            if (background != null) {
                this.mRefreshableView.setBackgroundDrawable(background);
            }
        } else if (a.hasValue(C1064R.styleable.PullToRefresh_ptrAdapterViewBackground)) {
            Utils.warnDeprecation("ptrAdapterViewBackground", "ptrRefreshableViewBackground");
            background = a.getDrawable(C1064R.styleable.PullToRefresh_ptrAdapterViewBackground);
            if (background != null) {
                this.mRefreshableView.setBackgroundDrawable(background);
            }
        }
        if (a.hasValue(C1064R.styleable.PullToRefresh_ptrOverScroll)) {
            this.mOverScrollEnabled = a.getBoolean(C1064R.styleable.PullToRefresh_ptrOverScroll, true);
        }
        if (a.hasValue(C1064R.styleable.PullToRefresh_ptrScrollingWhileRefreshingEnabled)) {
            this.mScrollingWhileRefreshingEnabled = a.getBoolean(C1064R.styleable.PullToRefresh_ptrScrollingWhileRefreshingEnabled, false);
        }
    ...
    private int getMaximumPullScroll() {
        ...
        switch (getPullToRefreshScrollDirection()) {
            case HORIZONTAL:
                round = Math.round(((float) getWidth()) / FRICTION);
                break;
            default:
                round = Math.round(((float) getHeight()) / FRICTION);
                break;
        }
        return dip2px(250.0f);
    }
    ...
    private final void smoothScrollToAndBack(int y) {
        smoothScrollTo(y, 200, 0, new C10594());
    }
}
