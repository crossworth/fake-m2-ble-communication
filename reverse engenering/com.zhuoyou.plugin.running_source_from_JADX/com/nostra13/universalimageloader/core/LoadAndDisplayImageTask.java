...
import com.nostra13.universalimageloader.utils.C1100L;
...
final class LoadAndDisplayImageTask implements Runnable, CopyListener {
    ...
    public void run() {
        if (!waitIfPaused() && !delayIfNeed()) {
            ...
            C1100L.m3301d(LOG_START_DISPLAY_IMAGE_TASK, this.memoryCacheKey);
            if (loadFromUriLock.isLocked()) {
                C1100L.m3301d(LOG_WAITING_FOR_IMAGE_LOADED, this.memoryCacheKey);
            }
            ...
            try {
                ...
                if (bmp == null || bmp.isRecycled()) {
                    ...
                    if (bmp != null) {
                        ...
                        if (this.options.shouldPreProcess()) {
                            C1100L.m3301d(LOG_PREPROCESS_IMAGE, this.memoryCacheKey);
                            bmp = this.options.getPreProcessor().process(bmp);
                            if (bmp == null) {
                                C1100L.m3302e(ERROR_PRE_PROCESSOR_NULL, this.memoryCacheKey);
                            }
                        }
                        if (bmp != null && this.options.isCacheInMemory()) {
                            C1100L.m3301d(LOG_CACHE_IMAGE_IN_MEMORY, this.memoryCacheKey);
                ...
                C1100L.m3301d(LOG_GET_IMAGE_FROM_MEMORY_CACHE_AFTER_WAITING, this.memoryCacheKey);
                if (bmp != null && this.options.shouldPostProcess()) {
                    C1100L.m3301d(LOG_POSTPROCESS_IMAGE, this.memoryCacheKey);
                    bmp = this.options.getPostProcessor().process(bmp);
                    if (bmp == null) {
                        C1100L.m3302e(ERROR_POST_PROCESSOR_NULL, this.memoryCacheKey);
                    }
                }
                checkTaskNotActual();
                checkTaskInterrupted();
    ...
    private boolean waitIfPaused() {
        ...
        if (pause.get()) {
            synchronized (this.engine.getPauseLock()) {
                if (pause.get()) {
                    C1100L.m3301d(LOG_WAITING_FOR_RESUME, this.memoryCacheKey);
                    try {
                        this.engine.getPauseLock().wait();
                        C1100L.m3301d(LOG_RESUME_AFTER_PAUSE, this.memoryCacheKey);
                    } catch (InterruptedException e) {
                        C1100L.m3302e(LOG_TASK_INTERRUPTED, this.memoryCacheKey);
                        return true;
                    }
                }
            }
    ...
    private boolean delayIfNeed() {
        ...
        C1100L.m3301d(LOG_DELAY_BEFORE_LOADING, Integer.valueOf(this.options.getDelayBeforeLoading()), this.memoryCacheKey);
        try {
    ...
    private Bitmap tryLoadBitmap() throws TaskCancelledException {
        ...
        try {
            ...
            if (imageFile != null && imageFile.exists() && imageFile.length() > 0) {
                C1100L.m3301d(LOG_LOAD_IMAGE_FROM_DISK_CACHE, this.memoryCacheKey);
                this.loadedFrom = LoadedFrom.DISC_CACHE;
                checkTaskNotActual();
                bitmap = decodeImage(Scheme.FILE.wrap(imageFile.getAbsolutePath()));
            }
            if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
                C1100L.m3301d(LOG_LOAD_IMAGE_FROM_NETWORK, this.memoryCacheKey);
    ...
    private boolean tryCacheImageOnDisk() throws TaskCancelledException {
        C1100L.m3301d(LOG_CACHE_IMAGE_ON_DISK, this.memoryCacheKey);
        try {
            ...
            C1100L.m3301d(LOG_RESIZE_CACHED_IMAGE_FILE, this.memoryCacheKey);
    ...
    private boolean downloadImage() throws IOException {
        ...
        if (is == null) {
            C1100L.m3302e(ERROR_NO_IMAGE_STREAM, this.memoryCacheKey);
        } else {
            try {
    ...
    private boolean resizeAndSaveImage(int maxWidth, int maxHeight) throws IOException {
        ...
        if (!(bmp == null || this.configuration.processorForDiskCache == null)) {
            C1100L.m3301d(LOG_PROCESS_IMAGE_BEFORE_CACHE_ON_DISK, this.memoryCacheKey);
            bmp = this.configuration.processorForDiskCache.process(bmp);
            if (bmp == null) {
                C1100L.m3302e(ERROR_PROCESSOR_FOR_DISK_CACHE_NULL, this.memoryCacheKey);
            }
        }
        if (bmp == null) {
            return false;
    ...
    private void fireCancelEvent() {
        if (!this.syncLoading && !isTaskInterrupted()) {
            runTask(new C10953(), false, this.handler, this.engine);
        }
    }
    ...
    private boolean isViewCollected() {
        ...
        C1100L.m3301d(LOG_TASK_CANCELLED_IMAGEAWARE_COLLECTED, this.memoryCacheKey);
        return true;
    }
    ...
    private boolean isViewReused() {
        ...
        C1100L.m3301d(LOG_TASK_CANCELLED_IMAGEAWARE_REUSED, this.memoryCacheKey);
        return true;
    }
    ...
    private boolean isTaskInterrupted() {
        ...
        C1100L.m3301d(LOG_TASK_INTERRUPTED, this.memoryCacheKey);
        return true;
    }
