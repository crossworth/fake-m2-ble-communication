...
public class SsoHandler {
    ...
    public void authorizeCallBack(int requestCode, int resultCode, Intent data) {
        LogUtil.m3307d(TAG, "requestCode: " + requestCode + ", resultCode: " + resultCode + ", data: " + data);
        String error;
        Bundle bundle;
        ...
        if (requestCode == this.mSSOAuthRequestCode) {
            if (resultCode == -1) {
                if (SecurityHelper.checkResponseAppLegal(this.mAuthActivity, this.mWeiboInfo, data)) {
                    ...
                    if (error == null) {
                        ...
                        if (accessToken == null || !accessToken.isSessionValid()) {
                            LogUtil.m3307d(TAG, "Failed to receive access token by SSO");
                            this.mWebAuthHandler.anthorize(this.mAuthListener);
                            return;
                        }
                        LogUtil.m3307d(TAG, "Login Success! " + accessToken.toString());
                        this.mAuthListener.onComplete(bundle);
                    } else if (error.equals("access_denied") || error.equals("OAuthAccessDeniedException")) {
                        LogUtil.m3307d(TAG, "Login canceled by user.");
                        ...
                        LogUtil.m3307d(TAG, "Login failed: " + error);
                ...
                if (data != null) {
                    LogUtil.m3307d(TAG, "Login failed: " + data.getStringExtra("error"));
            ...
            if (resultCode == -1) {
                ...
                if (data != null) {
                    ...
                    if (error != null) {
                        this.mAuthListener.onWeiboException(new WeiboDialogException(data.getStringExtra("error"), data.getIntExtra("error_code", -1), data.getStringExtra(NativeProtocol.BRIDGE_ARG_ERROR_DESCRIPTION)));
                        return;
                    }
                    return;
                }
