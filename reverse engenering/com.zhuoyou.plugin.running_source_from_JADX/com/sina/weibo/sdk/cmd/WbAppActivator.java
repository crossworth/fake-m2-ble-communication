...
public class WbAppActivator {
    ...
    private static class FrequencyHelper {
        ...
        public static long getFrequency(Context ctx, SharedPreferences sp) {
            if (sp != null) {
                return sp.getLong(KEY_FREQUENCY, 3600000);
            }
            return 3600000;
        }
    ...
    public void activateApp() {
        ...
        if (System.currentTimeMillis() - FrequencyHelper.getLastTime(this.mContext, sdkSp) < FrequencyHelper.getFrequency(this.mContext, sdkSp)) {
            LogUtil.m3310v(TAG, String.format("it's only %d ms from last time get cmd", new Object[]{Long.valueOf(period)}));
            return;
        }
        new Thread(new Runnable() {
            public void run() {
                WeiboException e;
                Throwable th;
                LogUtil.m3310v(WbAppActivator.TAG, "mLock.isLocked()--->" + WbAppActivator.this.mLock.isLocked());
                    ...
                    try {
                        ...
                        if (result != null) {
                            ...
                            try {
                                ...
                                cmds = cmds2;
                                try {
                                    LogUtil.m3308e(WbAppActivator.TAG, e.getMessage());
                                    WbAppActivator.this.mLock.unlock();
                                    if (cmds != null) {
                                        FrequencyHelper.saveFrequency(WbAppActivator.this.mContext, sdkSp, (long) cmds.getFrequency());
                                        FrequencyHelper.saveLastTime(WbAppActivator.this.mContext, sdkSp, System.currentTimeMillis());
                                    }
                                    LogUtil.m3310v(WbAppActivator.TAG, "after unlock \n mLock.isLocked()--->" + WbAppActivator.this.mLock.isLocked());
                                } catch (Throwable th2) {
                                    th = th2;
                                    WbAppActivator.this.mLock.unlock();
                                    if (cmds != null) {
    ...
    private static String requestCmdInfo(Context ctx, String appkey) {
        ...
        params.put(LogBuilder.KEY_APPKEY, appkey);
        params.put("packagename", pkgName);
        params.put("key_hash", keyHash);
        params.put("version", WBConstants.WEIBO_SDK_VERSION_CODE);
        return NetUtils.internalHttpRequest(ctx, "http://api.weibo.cn/2/client/common_config", Constants.HTTP_GET, params);
    }
