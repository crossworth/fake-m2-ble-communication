...
public class HttpManager {
    ...
    class C11251 implements RedirectHandler {
        C11251() {
        }

        public boolean isRedirectRequested(HttpResponse response, HttpContext context) {
            LogUtil.m3307d(HttpManager.TAG, "openUrl4RdirectURL isRedirectRequested method");
            return false;
        }

        public URI getLocationURI(HttpResponse response, HttpContext context) throws ProtocolException {
            LogUtil.m3307d(HttpManager.TAG, "openUrl4RdirectURL getLocationURI method");
            return null;
        }
    }

    class C11262 extends CustomRedirectHandler {
        C11262() {
        }
    ...
    public static String openUrl(Context context, String url, String method, WeiboParameters params) throws WeiboException {
        ...
        LogUtil.m3307d(TAG, "Response : " + ans);
        return ans;
    }
    ...
    private static HttpResponse requestHttpExecute(Context context, String url, String method, WeiboParameters params) {
        ...
        try {
            ...
            if (method.equals("GET")) {
                ...
                LogUtil.m3307d(TAG, "requestHttpExecute GET Url : " + url);
            } else {
                if (method.equals("POST")) {
                    LogUtil.m3307d(TAG, "requestHttpExecute POST Url : " + url);
                    ...
                    try {
                        if (params.hasBinaryData()) {
                            ...
                            LogUtil.m3307d(TAG, "requestHttpExecute POST postParam : " + postParam);
    ...
    public static String openUrl4RdirectURL(Context context, String url, String method, WeiboParameters params) throws WeiboException {
        ...
        try {
            ...
            client.setRedirectHandler(new C11251());
            setHttpCommonParam(context, params);
            HttpUriRequest request = null;
            client.getParams().setParameter("http.route.default-proxy", NetStateManager.getAPN());
            if (method.equals("GET")) {
                url = new StringBuilder(String.valueOf(url)).append("?").append(params.encodeUrl()).toString();
                LogUtil.m3307d(TAG, "openUrl4RdirectURL GET url : " + url);
                request = new HttpGet(url);
            } else if (method.equals("POST")) {
                HttpPost post = new HttpPost(url);
                LogUtil.m3307d(TAG, "openUrl4RdirectURL POST url : " + url);
                Object request2 = post;
            }
            HttpResponse response = client.execute(request);
            int statusCode = response.getStatusLine().getStatusCode();
            if (statusCode == 301 || statusCode == HttpResponseCode.FOUND) {
                ...
                LogUtil.m3307d(TAG, "RedirectURL = " + redirectURL);
                shutdownHttpClient(client);
    ...
    public static String openRedirectUrl4LocationUri(Context context, String url, String method, WeiboParameters params) {
        ...
        DefaultHttpClient client = null;
        try {
            CustomRedirectHandler redirectHandler = new C11262();
            try {
                client = (DefaultHttpClient) getNewHttpClient();
        ...
        synchronized (HttpManager.class) {
            ...
            if (filePath.exists()) {
                ...
                try {
                    ...
                    Header header = response.getFirstHeader(Constants.HTTP_HEADER_CONTENT_ENCODING);
                    if (header != null) {
                        if (header.getValue().toLowerCase().indexOf(Constants.GZIP_TAG) > -1) {
    ...
    public static String readRsponse(HttpResponse response) throws WeiboException {
        ...
        try {
            inputStream = entity.getContent();
            Header header = response.getFirstHeader(Constants.HTTP_HEADER_CONTENT_ENCODING);
            if (header != null && header.getValue().toLowerCase().indexOf(Constants.GZIP_TAG) > -1) {
            ...
            LogUtil.m3307d(TAG, "readRsponse result : " + result);
            if (inputStream != null) {
                try {
                    inputStream.close();
                } catch (IOException e) {
    ...
    private static SSLSocketFactory getSSLSocketFactory() {
        if (sSSLSocketFactory == null) {
            try {
                ...
                LogUtil.m3307d(TAG, "getSSLSocketFactory noraml !!!!!");
            } catch (Exception e) {
                e.printStackTrace();
                sSSLSocketFactory = SSLSocketFactory.getSocketFactory();
                LogUtil.m3307d(TAG, "getSSLSocketFactory error default !!!!!");
            }
        }
