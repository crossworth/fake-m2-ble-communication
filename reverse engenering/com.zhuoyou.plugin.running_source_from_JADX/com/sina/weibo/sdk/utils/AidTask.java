...
public class AidTask {
    ...
    class C11382 implements Runnable {
        ...
        public void run() {
                ...
                if (aidInfo == null) {
                    ...
                    do {
                        ...
                        try {
                            ...
                            LogUtil.m3308e(AidTask.TAG, "AidTaskInit WeiboException Msg : " + e.getMessage());
                            if (retry >= 3) {
                            }
                        }
                    } while (retry >= 3);
            ...
            LogUtil.m3308e(AidTask.TAG, "tryLock : false, return");
        }
    }
        ...
        public static AidInfo parseJson(String response) throws WeiboException {
            ...
            try {
                ...
                if (resObj.has("error") || resObj.has("error_code")) {
                    LogUtil.m3307d(AidTask.TAG, "loadAidFromNet has error !!!");
                ...
                LogUtil.m3307d(AidTask.TAG, "loadAidFromNet JSONException Msg : " + e.getMessage());
                throw new WeiboException("loadAidFromNet has error !!!");
            }
        }
    ...
    public void aidTaskInit(String appKey) {
        if (!TextUtils.isEmpty(appKey)) {
            LogUtil.m3308e(TAG, "aidTaskInit ");
            initAidInfo(appKey);
        }
    }
    ...
    private String loadAidFromNet() throws WeiboException {
        ...
        params.put(LogBuilder.KEY_APPKEY, this.mAppKey);
        params.put("mfp", mfp);
        params.put("packagename", pkgName);
        params.put("key_hash", keyHash);
        try {
            String response = NetUtils.internalHttpRequest(this.mContext, "https://api.weibo.com/oauth2/getaid.json", Constants.HTTP_GET, params);
            LogUtil.m3307d(TAG, "loadAidFromNet response : " + response);
            return response;
        } catch (WeiboException e) {
            LogUtil.m3307d(TAG, "loadAidFromNet WeiboException Msg : " + e.getMessage());
            throw e;
        }
    }
    ...
    private static String getMfp(Context ctx) {
        ...
        LogUtil.m3307d(TAG, "genMfpString() utf-8 string : " + mfpJsonUtf8);
        try {
            ...
            LogUtil.m3307d(TAG, "encryptRsa() string : " + rsaMfp);
            return rsaMfp;
        } catch (Exception e2) {
            LogUtil.m3308e(TAG, e2.getMessage());
            return "";
        }
    }
    ...
    private static String genMfpString(Context ctx) {
        ...
        try {
            ...
            if (!TextUtils.isEmpty(serial)) {
                mfpObj.put(Constants.VIA_SHARE_TYPE_PUBLISHMOOD, serial);
            }
        ...
        try {
            ...
            while (true) {
                ...
                try {
                    ...
                    bos2.write(enBytes);
                    LogUtil.m3307d(TAG, "encryptRsa offset = " + offset + "     len = " + len + "     enBytes len = " + enBytes.length);
                    offset += len;
                } catch (Throwable th2) {
                    th = th2;
                    bos = bos2;
            ...
            LogUtil.m3307d(TAG, "encryptRsa total enBytes len = " + enBytes.length);
            byte[] base64byte = Base64.encodebyte(enBytes);
            LogUtil.m3307d(TAG, "encryptRsa total base64byte len = " + base64byte.length);
            String VERSION = "01";
            String base64string = "01" + new String(base64byte, "UTF-8");
            LogUtil.m3307d(TAG, "encryptRsa total base64string : " + base64string);
            if (bos2 != null) {
                try {
                    bos2.close();
                } catch (IOException e) {
    ...
    private static String getAndroidId(Context ctx) {
        try {
            return Secure.getString(ctx.getContentResolver(), SocializeProtocolConstants.PROTOCOL_KEY_ANDROID_ID);
        } catch (Exception e) {
            return "";
        }
    }
