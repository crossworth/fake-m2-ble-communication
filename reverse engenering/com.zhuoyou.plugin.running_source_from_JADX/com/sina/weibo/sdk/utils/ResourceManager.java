...
public class ResourceManager {
    ...
    private static String getAppropriatePathOfDrawable(Context context, String fileName) {
        if (TextUtils.isEmpty(fileName)) {
            LogUtil.m3308e(TAG, "id is NOT correct!");
            return null;
        }
        int properDpi;
        String currentDpi = getCurrentDpiFolder(context);
        LogUtil.m3307d(TAG, "find Appropriate path...");
        ...
        if (existIndexLeftDpi <= 0 || existIndexRightDpi <= 0) {
            if (existIndexLeftDpi > 0 && existIndexRightDpi < 0) {
                ...
                properDpi = -1;
                LogUtil.m3308e(TAG, "Not find the appropriate path for drawable");
            } else {
    ...
    private static boolean isFileExisted(Context context, String filePath) {
        ...
        try {
            ...
            LogUtil.m3307d(TAG, "file [" + filePath + "] existed");
            if (is != null) {
                try {
                    is.close();
                } catch (IOException e) {
            ...
            LogUtil.m3307d(TAG, "file [" + filePath + "] NOT existed");
            if (is == null) {
                return false;
            }
            try {
