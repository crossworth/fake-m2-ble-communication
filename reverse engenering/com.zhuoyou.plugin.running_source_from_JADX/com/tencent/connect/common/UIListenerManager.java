...
public class UIListenerManager {
    ...
    public Object setListenerWithRequestcode(int i, IUiListener iUiListener) {
        ...
        if (actionFromRequestcode == null) {
            C1314f.m3872e(TAG, "setListener action is null! rquestCode=" + i);
            return null;
        }
    ...
    public Object setListnerWithAction(String str, IUiListener iUiListener) {
        ...
        if (requestCodeFromCallback == -1) {
            C1314f.m3872e(TAG, "setListnerWithAction fail, action = " + str);
            return null;
        }
        synchronized (this.mListenerMap) {
    ...
    public IUiListener getListnerWithRequestCode(int i) {
        ...
        C1314f.m3872e(TAG, "getListner action is null! rquestCode=" + i);
        return null;
    }

    public IUiListener getListnerWithAction(String str) {
        if (str == null) {
            C1314f.m3872e(TAG, "getListnerWithAction action is null!");
            return null;
        }
        synchronized (this.mListenerMap) {
    ...
    public void handleDataToListener(Intent intent, IUiListener iUiListener) {
        C1314f.m3870c(TAG, "handleDataToListener");
        if (intent == null) {
            ...
            if (intExtra == 0) {
                ...
                if (stringExtra2 != null) {
                    try {
                        ...
                        C1314f.m3868b(TAG, "OpenUi, onActivityResult, json error", e);
                        return;
                    }
                }
                C1314f.m3867b(TAG, "OpenUi, onActivityResult, onComplete");
                iUiListener.onComplete(new JSONObject());
                return;
            }
            C1314f.m3872e(TAG, "OpenUi, onActivityResult, onError = " + intExtra + "");
    ...
    private IUiListener buildListener(int i, IUiListener iUiListener) {
        if (i == 11101) {
            C1314f.m3872e(TAG, "登录的接口回调不能重新构建，暂时无法提供，先记录下来这种情况是否存在");
        } else if (i == Constants.REQUEST_SOCIAL_API) {
            C1314f.m3872e(TAG, "Social Api 的接口回调需要使用param来重新构建，暂时无法提供，先记录下来这种情况是否存在");
        } else if (i == Constants.REQUEST_SOCIAL_H5) {
            C1314f.m3872e(TAG, "Social Api 的H5接口回调需要使用param来重新构建，暂时无法提供，先记录下来这种情况是否存在");
        }
        return iUiListener;
    }
    ...
    public boolean onActivityResult(int i, int i2, Intent intent, IUiListener iUiListener) {
        ...
        C1314f.m3870c(TAG, "onActivityResult req=" + i + " res=" + i2);
        ...
        if (listnerWithRequestCode != null) {
            ...
            C1314f.m3872e(TAG, "onActivityResult can't find the listener");
            return false;
        }
        if (i2 != -1) {
