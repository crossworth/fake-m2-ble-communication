...
public class ServerSetting {
    ...
    private static ServerSetting f4214a = null;
    private volatile WeakReference<SharedPreferences> f4215b = null;

    public static synchronized ServerSetting getInstance() {
        ServerSetting serverSetting;
        synchronized (ServerSetting.class) {
            if (f4214a == null) {
                f4214a = new ServerSetting();
            }
            serverSetting = f4214a;
        }
    ...
    public String getEnvUrl(Context context, String str) {
        if (this.f4215b == null || this.f4215b.get() == null) {
            this.f4215b = new WeakReference(context.getSharedPreferences("ServerPrefs", 0));
        }
        try {
            Object host = new URL(str).getHost();
            if (host == null) {
                C1314f.m3872e("openSDK_LOG.ServerSetting", "Get host error. url=" + str);
            } else {
                Object string = ((SharedPreferences) this.f4215b.get()).getString(host, null);
                if (string == null || host.equals(string)) {
                    C1314f.m3864a("openSDK_LOG.ServerSetting", "host=" + host + ", envHost=" + string);
                } else {
                    str = str.replace(host, string);
                    C1314f.m3864a("openSDK_LOG.ServerSetting", "return environment url : " + str);
                }
            }
        } catch (Exception e) {
            C1314f.m3872e("openSDK_LOG.ServerSetting", "getEnvUrl url=" + str + "error.: " + e.getMessage());
        }
        return str;
    }

    public void setEnvironment(Context context, int i) {
        if (context != null && (this.f4215b == null || this.f4215b.get() == null)) {
            this.f4215b = new WeakReference(context.getSharedPreferences("ServerPrefs", 0));
        }
        if (i == 0 || i == 1) {
            Editor edit;
            switch (i) {
                case 0:
                    edit = ((SharedPreferences) this.f4215b.get()).edit();
                    if (edit != null) {
        ...
        C1314f.m3872e("openSDK_LOG.ServerSetting", "切换环境参数错误，正式环境为0，体验环境为1");
    }
}
