...
final class AccessTokenManager {
    ...
    private boolean shouldExtendAccessToken() {
        ...
        if (!this.currentAccessToken.getSource().canExtendToken() || now.longValue() - this.lastAttemptedTokenExtendDate.getTime() <= 3600000 || now.longValue() - this.currentAccessToken.getLastRefresh().getTime() <= LogBuilder.MAX_INTERVAL) {
            return false;
        }
        return true;
    }
    ...
    private static GraphRequest createExtendAccessTokenRequest(AccessToken accessToken, Callback callback) {
        ...
        parameters.putString(WBConstants.AUTH_PARAMS_GRANT_TYPE, "fb_extend_sso_token");
    ...
    void refreshCurrentAccessToken() {
        if (Looper.getMainLooper().equals(Looper.myLooper())) {
            ...
            new Handler(Looper.getMainLooper()).post(new C14761());
        }
    }
