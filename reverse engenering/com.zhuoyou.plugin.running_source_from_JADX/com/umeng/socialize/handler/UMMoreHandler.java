...
public class UMMoreHandler extends UMSSOHandler {
    ...
    public boolean share(ShareContent content, UMShareListener listener) {
        ...
        if (content.mMedia == null || !(content.mMedia instanceof UMImage)) {
            ...
            UMImage image = content.mMedia;
            if (image.asFileImage().getPath() != null) {
                share_intent.putExtra("android.intent.extra.STREAM", SocializeUtils.insertImage(getContext(), image.asFileImage().getPath()));
            }
        }
        share_intent.putExtra("android.intent.extra.SUBJECT", content.mTitle);
        ...
        share_intent.addFlags(268435456);
        try {
