...
public class TouchImageView extends ImageView {
    ...
    private class Fling implements Runnable {
        ...
        Fling(int velocityX, int velocityY) {
            ...
            TouchImageView.this.matrix.getValues(TouchImageView.this.f5011m);
            int startX = (int) TouchImageView.this.f5011m[2];
            int startY = (int) TouchImageView.this.f5011m[5];
    ...
    private void savePreviousImageValues() {
        if (this.matrix != null && this.viewHeight != 0 && this.viewWidth != 0) {
            this.matrix.getValues(this.f5011m);
            this.prevMatrix.setValues(this.f5011m);
    ...
    public Parcelable onSaveInstanceState() {
        ...
        this.matrix.getValues(this.f5011m);
        bundle.putFloatArray("matrix", this.f5011m);
    ...
    public void onRestoreInstanceState(Parcelable state) {
        if (state instanceof Bundle) {
            ...
            this.f5011m = bundle.getFloatArray("matrix");
            this.prevMatrix.setValues(this.f5011m);
    ...
    public void setZoom(float scale, float focusX, float focusY, ScaleType scaleType) {
        if (this.onDrawReady) {
            ...
            this.matrix.getValues(this.f5011m);
            this.f5011m[2] = -((getImageWidth() * focusX) - (((float) this.viewWidth) * 0.5f));
            this.f5011m[5] = -((getImageHeight() * focusY) - (((float) this.viewHeight) * 0.5f));
            this.matrix.setValues(this.f5011m);
            fixTrans();
            setImageMatrix(this.matrix);
            return;
        }
    ...
    private void fixTrans() {
        this.matrix.getValues(this.f5011m);
        float transX = this.f5011m[2];
        float transY = this.f5011m[5];
    ...
    private void fixScaleTrans() {
        ...
        if (getImageWidth() < ((float) this.viewWidth)) {
            this.f5011m[2] = (((float) this.viewWidth) - getImageWidth()) / 2.0f;
        }
        if (getImageHeight() < ((float) this.viewHeight)) {
            this.f5011m[5] = (((float) this.viewHeight) - getImageHeight()) / 2.0f;
        }
        this.matrix.setValues(this.f5011m);
    }
    ...
    private void fitImageToView() {
        ...
        if (drawable != null && drawable.getIntrinsicWidth() != 0 && drawable.getIntrinsicHeight() != 0 && this.matrix != null && this.prevMatrix != null) {
            ...
            switch (C19581.$SwitchMap$android$widget$ImageView$ScaleType[this.mScaleType.ordinal()]) {
                case 1:
                    scaleY = 1.0f;
                    scaleX = 1.0f;
                    break;
            ...
            if (isZoomed() || this.imageRenderedAtLeastOnce) {
                ...
                this.prevMatrix.getValues(this.f5011m);
                this.f5011m[0] = (this.matchViewWidth / ((float) drawableWidth)) * this.normalizedScale;
                this.f5011m[4] = (this.matchViewHeight / ((float) drawableHeight)) * this.normalizedScale;
                float transX = this.f5011m[2];
                float transY = this.f5011m[5];
    ...
    private void translateMatrixAfterRotate(int axis, float trans, float prevImageSize, float imageSize, int prevViewSize, int viewSize, int drawableSize) {
        if (imageSize < ((float) viewSize)) {
            this.f5011m[axis] = (((float) viewSize) - (((float) drawableSize) * this.f5011m[0])) * 0.5f;
        } else if (trans > 0.0f) {
            this.f5011m[axis] = -((imageSize - ((float) viewSize)) * 0.5f);
        } else {
            this.f5011m[axis] = -((((Math.abs(trans) + (((float) prevViewSize) * 0.5f)) / prevImageSize) * imageSize) - (((float) viewSize) * 0.5f));
        }
    }
    ...
    private PointF transformCoordTouchToBitmap(float x, float y, boolean clipToBitmap) {
        ...
        float finalY = ((y - this.f5011m[5]) * origH) / getImageHeight();
        if (clipToBitmap) {
    ...
    private PointF transformCoordBitmapToTouch(float bx, float by) {
        this.matrix.getValues(this.f5011m);
        return new PointF(this.f5011m[2] + (getImageWidth() * (bx / ((float) getDrawable().getIntrinsicWidth()))), this.f5011m[5] + (getImageHeight() * (by / ((float) getDrawable().getIntrinsicHeight()))));
    }
