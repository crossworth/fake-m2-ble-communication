package com.zy.makeramen.roundedimageview;
...
public class RoundedImageView extends ImageView {
    ...
    public RoundedImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        this.mTileModeX = DEFAULT_TILE_MODE;
        this.mTileModeY = DEFAULT_TILE_MODE;
        TypedArray a = context.obtainStyledAttributes(attrs, C1966R.styleable.RoundedImageView, defStyle, 0);
        int index = a.getInt(C1966R.styleable.RoundedImageView_android_scaleType, -1);
        if (index >= 0) {
            setScaleType(SCALE_TYPES[index]);
        } else {
            setScaleType(ScaleType.FIT_CENTER);
        }
        float cornerRadiusOverride = (float) a.getDimensionPixelSize(C1966R.styleable.RoundedImageView_riv_corner_radius, -1);
        this.mCornerRadii[0] = (float) a.getDimensionPixelSize(C1966R.styleable.RoundedImageView_riv_corner_radius_top_left, -1);
        this.mCornerRadii[1] = (float) a.getDimensionPixelSize(C1966R.styleable.RoundedImageView_riv_corner_radius_top_right, -1);
        this.mCornerRadii[2] = (float) a.getDimensionPixelSize(C1966R.styleable.RoundedImageView_riv_corner_radius_bottom_right, -1);
        this.mCornerRadii[3] = (float) a.getDimensionPixelSize(C1966R.styleable.RoundedImageView_riv_corner_radius_bottom_left, -1);
        boolean any = false;
        ...
        this.mBorderWidth = (float) a.getDimensionPixelSize(C1966R.styleable.RoundedImageView_riv_border_width, -1);
        if (this.mBorderWidth < 0.0f) {
            this.mBorderWidth = 0.0f;
        }
        this.mBorderColor = a.getColorStateList(C1966R.styleable.RoundedImageView_riv_border_color);
        if (this.mBorderColor == null) {
            this.mBorderColor = ColorStateList.valueOf(-16777216);
        }
        this.mMutateBackground = a.getBoolean(C1966R.styleable.RoundedImageView_riv_mutate_background, false);
        this.mIsOval = a.getBoolean(C1966R.styleable.RoundedImageView_riv_oval, false);
        int tileMode = a.getInt(C1966R.styleable.RoundedImageView_riv_tile_mode, -2);
        if (tileMode != -2) {
            setTileModeX(parseTileMode(tileMode));
            setTileModeY(parseTileMode(tileMode));
        }
        int tileModeX = a.getInt(C1966R.styleable.RoundedImageView_riv_tile_mode_x, -2);
        if (tileModeX != -2) {
            setTileModeX(parseTileMode(tileModeX));
        }
        int tileModeY = a.getInt(C1966R.styleable.RoundedImageView_riv_tile_mode_y, -2);
        if (tileModeY != -2) {
    ...
    public void setScaleType(ScaleType scaleType) {
        if (!$assertionsDisabled && scaleType == null) {
            ...
            switch (C19681.$SwitchMap$android$widget$ImageView$ScaleType[scaleType.ordinal()]) {
                case 1:
                case 2:
                case 3:
                case 4:
    ...
    private void updateAttrs(Drawable drawable) {
        if (drawable != null) {
            if (drawable instanceof RoundedDrawable) {
                ((RoundedDrawable) drawable).setScaleType(this.mScaleType).setBorderWidth(this.mBorderWidth).setBorderColor(this.mBorderColor).setOval(this.mIsOval).setTileModeX(this.mTileModeX).setTileModeY(this.mTileModeY);
