...
class AsyncOperationExecutor implements Runnable, Callback {
    ...
    public void run() {
        while (true) {
            try {
                ...
                DaoLog.m4570w(Thread.currentThread().getName() + " was interruppted", e);
                this.executorRunning = false;
                return;
            } catch (Throwable th) {
                this.executorRunning = false;
    ...
    private void mergeTxAndExecute(AsyncOperation operation1, AsyncOperation operation2) {
        ...
        while (i < mergedOps.size()) {
            try {
                ...
                try {
                    ...
                    DaoLog.m4566i("Async transaction could not be ended, success so far was: " + success, e);
                    success = false;
                }
            }
        }
        ...
        DaoLog.m4565i("Reverted merged transaction because one of the operations failed. Executing operations one by one instead...");
        i$ = mergedOps.iterator();
        while (i$.hasNext()) {
    ...
    private void executeOperation(de.greenrobot.dao.async.AsyncOperation r5) {
        ...
        r5.timeStarted = r2;
        r1 = de.greenrobot.dao.async.AsyncOperationExecutor.C19691.$SwitchMap$de$greenrobot$dao$async$AsyncOperation$OperationType;	 Catch:{ Throwable -> 0x002e }
