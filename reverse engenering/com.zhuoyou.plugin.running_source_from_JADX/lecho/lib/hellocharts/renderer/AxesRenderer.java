...
public class AxesRenderer {
    ...
    private void initAxisDimensionForTiltedLabels(int position) {
        ...
        this.labelDimensionForMarginsTab[position] = pythagoreanFromAscent + ((int) Math.sqrt(Math.pow((double) this.labelWidthTab[position], 2.0d) / 2.0d));
    ...
    private void prepareCustomAxis(Axis axis, int position) {
        ...
        int module = (int) Math.max(WeightedLatLng.DEFAULT_INTENSITY, Math.ceil((((double) (axis.getValues().size() * this.labelDimensionForStepsTab[position])) * 1.5d) / ((double) scale)));
