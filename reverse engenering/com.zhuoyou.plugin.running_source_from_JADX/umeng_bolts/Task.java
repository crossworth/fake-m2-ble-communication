...
public class Task<TResult> {
    ...
    class C21871 implements Continuation<TResult, Task<Void>> {
        C21871() {
        }
    ...
    public Task<Void> makeVoid() {
        return continueWithTask(new C21871());
    }
                    ...
                    if (count.decrementAndGet() == 0) {
                        if (causes.size() != 0) {
                            if (causes.size() == 1) {
                                ...
                                allFinished.setError(new AggregateException(String.format("There were %d exceptions.", new Object[]{Integer.valueOf(causes.size())}), (Throwable[]) causes.toArray(new Throwable[causes.size()])));
                            }
    ...
    private static <TContinuationResult, TResult> void completeAfterTask(final TaskCompletionSource tcs, final Continuation<TResult, Task<TContinuationResult>> continuation, final Task<TResult> task, Executor executor) {
        executor.execute(new Runnable() {
            ...
            class C21861 implements Continuation<TContinuationResult, Void> {
                C21861() {
                }
            ...
            public void run() {
                try {
                    ...
                    if (result == null) {
                        ...
                        result.continueWith(new C21861());
                    }
                } catch (Exception e) {
                    tcs.setError(e);
                }
