...
import umeng_bolts.AppLink.Target;
import umeng_bolts.Task.TaskCompletionSource;
...
public class WebViewAppLinkResolver implements AppLinkResolver {
    ...
    public Task<AppLink> getAppLinkFromUrlInBackground(final Uri url) {
        ...
        return Task.callInBackground(new Callable<Void>() {
            public Void call() throws Exception {
                ...
                while (currentURL != null) {
                    ...
                    if (connection instanceof HttpURLConnection) {
                        ...
                        if (httpConnection.getResponseCode() < 300 || httpConnection.getResponseCode() >= HttpResponseCode.BAD_REQUEST) {
                            currentURL = null;
                        } else {
            ...
            class C21971 extends WebViewClient {
            ...
            public Task<JSONArray> then(Task<Void> task) throws Exception {
                ...
                webView.setWebViewClient(new C21971());
    ...
    private static AppLink makeAppLinkFromAlData(Map<String, Object> appLinkDict, Uri destination) {
        ...
        for (Map<String, Object> platformMap : platformMapList) {
            ...
            for (int i = 0; i < maxCount; i++) {
                Object obj;
                if (urls.size() > i) {
                    obj = ((Map) urls.get(i)).get(KEY_AL_VALUE);
                } else {
                    obj = null;
                }
                Uri url = tryCreateUrl((String) obj);
                if (packages.size() > i) {
                    obj = ((Map) packages.get(i)).get(KEY_AL_VALUE);
                } else {
                    obj = null;
                }
                String packageName = (String) obj;
                if (classes.size() > i) {
                    obj = ((Map) classes.get(i)).get(KEY_AL_VALUE);
                } else {
                    obj = null;
                }
                String className = (String) obj;
                if (appNames.size() > i) {
                    obj = ((Map) appNames.get(i)).get(KEY_AL_VALUE);
                } else {
                    obj = null;
                }
                targets.add(new Target(packageName, className, url, (String) obj));
            }
        }
        ...
        if (webMapList != null && webMapList.size() > 0) {
            ...
            if (shouldFallbacks != null && shouldFallbacks.size() > 0) {
                if (Arrays.asList(new String[]{"no", "false", "0"}).contains(((String) ((Map) shouldFallbacks.get(0)).get(KEY_AL_VALUE)).toLowerCase())) {
                    webUrl = null;
                }
            }
